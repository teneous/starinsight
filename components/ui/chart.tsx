...
export function ChartContainer({ className, children, config, ...props }: ChartRootProps) {
  const style = React.useMemo(() => {
    return Object.entries(config).reduce(
      (styles, [key, value]) => {
        if (value.color) {
          styles[`--color-${key}`] =
            typeof value.color === 'string' ? value.color : value.color.light;
        }
        return styles;
      },
...
export function ChartTooltipContent({ className, ...props }: TooltipContentProps) {
  return (
    <div className={cn('rounded-md border bg-background p-2.5 shadow-sm', className)} {...props} />
  );
}

export function ChartLegend(props: any) {
  return <Legend {...props} layout="horizontal" verticalAlign="bottom" align="center" />;
}
...
export function ChartLegendContent({ className, ...props }: LegendContentProps) {
  return (
      ...
      return (
        <div className="rounded-lg border bg-zinc-900/95 p-3 shadow-xl backdrop-blur-sm">
          <div className="flex flex-col gap-1">
            <span className="text-sm font-medium text-zinc-200">{label || payload[0].name}</span>
